...
- **FE**: pixel flood fill, drag-to-group UI elements, connected node selection in graph editors,
  undo grouping
- **BE**: connected components, cycle detection, Kruskal's MST, social network clustering,
  equivalence classes, network partition detection
 ...
 * connected(0, 2) → true
 * dimension → 3 (distinct sets: {0,1,2,3}, {4}, {5})
 */
ds.find(0) === ds.find(3);
ds.find(0) === ds.find(4);
